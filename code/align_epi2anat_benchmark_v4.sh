...
#               - Auto-detects prepared fieldmaps or accepts -nf flag to skip.
...
#
# Usage:
#   ./align_epi2anat_benchmark_v4.sh -d <site_dir> -s <subj_id> -a <aw_dir> [-ses <session_str>] [-nf]
...
==============================================================================
  MULTI-SESSION ALIGNMENT BENCHMARKING (EPI -> ANAT) [V4 - FMAP] - HELP
==============================================================================
...
Usage:
  $0 -d <site_dir> -s <subj_id> -a <aw_dir> [-ses <session_string>] [-nf]
  $0 -h
...
Optional Flags:
  ...
  -nf       : No Fieldmap flag. Force skipping fieldmap unwarping even if
              prepared fieldmaps are present.
  -dwell <s>: Custom EPI Dwell Time / Effective Echo Spacing in seconds 
              (Default: auto-detected from BOLD JSON or fallback to 0.00065s).
    ...
    # Wrap session execution into a block piped directly to 'tee'
    {
        ...
        if [ "$disable_fmap" = true ]; then
            ...
            if [ -n "$user_dwell" ]; then
                ...
                # Try finding sidecar JSON of the selected BOLD run
...
" 2>/dev/null)
            ...
            # Run FSL fugue to correct geometric distortion on the base EPI volume
