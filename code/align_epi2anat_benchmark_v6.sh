...
# ==============================================================================
# Script Name : align_epi2anat_benchmark_v6.sh
# Language    : BASH
...
#               - Conditional B0 Fieldmap Unwarping support (FSL fugue).
#               - Anatomical center alignment with @Align_Centers.
#               - Heuristic search limits (-big_move, -giant_move, -ginormous_move).
#               - Multi-cost function testing (10 costs in rigid-body 6 DOF).
#               - Automated Dice spatial overlap ranking and top-3 @AddEdge.
#               - Per-test and cross-anatomical summary reports (.tsv).
#
# Usage:
#   ./align_epi2anat_benchmark_v6.sh -d <site_dir> -s <subj_id> -a <aw_dir> [options]
#   ./align_epi2anat_benchmark_v6.sh -h
# ==============================================================================
...
==============================================================================
 MULTI-SITE / MULTI-ANAT ALIGNMENT BENCHMARK (EPI -> ANAT) [V6] - HELP
==============================================================================

Description:
  Automates multi-cost function testing across all available anatomical sessions
  and runs for NHP fMRI in AFNI:
    - Automatically discovers and evaluates 1 or ALL T1w anatomical runs/sessions.
    - Evaluates geometric unwarping (FSL fugue) via prepared fieldmaps if present.
...
Optional Flags:
  ...
  -nf, --no-fmap       : No Fieldmap flag. Force skipping fieldmap unwarping.
...
# Deduplicate prefixes to avoid testing both .nii.gz and .HEAD if both exist
...
echo "=================================================="
echo " STARTING MULTI-ANAT / MULTI-RUN BENCHMARK [V6]"
...
echo " Fieldmap Mode      : $( [ "$disable_fmap" = true ] && echo "DISABLED (-nf)" || echo "AUTO-DETECT (DEFAULT)" )"
...
printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    ...
    "active_move" "best_cost" "best_dice" "anat_source_path" > "$master_report"
...
for f_ses in "${func_sessions[@]}"; do
    ...
    # Prep directory for base TR & Fieldmap
    ...
    # Fieldmap Geometric Unwarping (FSL FUGUE)
    ...
    if [ "$disable_fmap" = true ]; then
        echo "[INFO] Fieldmap correction disabled by user (-nf flag)."
...
" 2>/dev/null)
    ...
    for raw_anat_path in "${anat_target_list[@]}"; do
...

        {
            ...
            echo "  Target Anatomy File : $raw_anat_path"
...
")
            ...
            # Export local TSV report
            {
                printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
                    ...
                    "cost_function" "dice_coefficient" "is_best_cost" "fieldmap_applied" "anat_source"
                ...
                while read -r d_score c_name dset_path; do
                    ...
                    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
                        ...
                        "$c_name" "$d_score" "$is_best" "$fmap_applied" "$raw_anat_path"
                done < "$dice_results_file"
            } > "$tsv_report"
            ...
            printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
                ...
                "${active_move:-none}" "$best_cost_name" "$best_dice_score" "$raw_anat_path" >> "$master_report"
...
# ==============================================================================
# GLOBAL SUMMARY DISPLAY & WINNER SELECTION
# ==============================================================================
echo -e "\n======================================================================"
echo "          MULTI-ANATOMICAL BENCHMARKING GLOBAL SUMMARY                "
echo "======================================================================"
