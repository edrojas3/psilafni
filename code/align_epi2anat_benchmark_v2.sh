...
#               Pre-centers anatomy with @Align_Centers and evaluates 
#               multiple cost functions in rigid-body (6 DOF).
#
# Usage:
...
Description:
  Automates multi-cost function testing per session for NHP fMRI in AFNI:
    ...
    - Forces rigid-body (6 DOF: shift + rotate) across all cost functions.
...
for ses in "${sessions[@]}"; do
    ...
    # Set up session-specific paths (Using dedicated 'align_tests_centered' directory)
    if [ "$ses" == "no_ses" ]; then
